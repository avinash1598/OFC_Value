...
for i = 1:length(folders)
    ...
    sessionPath = fullfile(sessionPath, "pvalue_LOFC_vs_ROFC.mat");
...
for i = 1:length(folders)
    ...
    session = folders(i).name;
    sessionFilePath = fullfile(dataBaseDIR, session, "pvalue_LOFC_vs_ROFC.mat");

    % Check if all the time bins are same
    regressionData = load(sessionFilePath); 
    
    pval_ValueLOFC_ = regressionData.dataToSave.LOFC.pValTrialValue; % TODO: chqange
